...
import { RATING_PAGE_DATA_URLs } from '../../../constants';
...
export class RatingPageService {
  ...
  getData(parameter: IReviewPageParameter, planId: number): Observable<IApiResult> {
    // console.log('GetData', this.getDataUrl(parameter, parameter.role, planId, ''));
  ...
  completeSection(parameter: IReviewPageParameter, pageId: number, planId: number): Observable<IApiResult> {
    // console.log('completeSection - Rating ---1234599', this.getCompleteSectionUrl(parameter, parameter.role, pageId, planId));
  ...
  private getSaveAndContinueUrl(parameter: IReviewPageParameter, roleType: number, planId: number): string {
    ...
    switch (roleType) {
      case RoleTypes.EMPLOYEE:
        ...
        return `${RATING_PAGE_DATA_URLs.saveAndContinue}/${planId}/${assetId}`;
      case RoleTypes.LINE_MANAGER:
        return `${RATING_PAGE_DATA_URLs.lineManagerSaveAndContinue}/${employeeId}/${planId}/${assetId}`; //
      case RoleTypes.EMPLOYEE_ACCEPT_REJECT:
        return ``;
      case RoleTypes.REVIEWER_ASSESSING:
        return `${RATING_PAGE_DATA_URLs.lineManagerTwoSaveAndContinue}/${employeeId}/${planId}/${assetId}`; //
  ...
  private getCompleteSectionUrl(parameter: IReviewPageParameter, roleType: number, pageId: number, planId: number): string {
    ...
    switch (roleType) {
      case RoleTypes.EMPLOYEE:
        return `${RATING_PAGE_DATA_URLs.completeSection}/${pageId}/${planId}`;
      case RoleTypes.LINE_MANAGER:
        return `${RATING_PAGE_DATA_URLs.lineManagerCompleteSection}/${pageId}/${planId}/${employeeId}`;
      case RoleTypes.EMPLOYEE_ACCEPT_REJECT:
        return ``;
      case RoleTypes.REVIEWER_ASSESSING:
        return `${RATING_PAGE_DATA_URLs.lineManagerTwoCompleteSection}/${pageId}/${planId}/${employeeId}`;
      case RoleTypes.REVIEWER_REVIEWING:
        return ``;
      case RoleTypes.MODERATION:
        return ``;
  ...
  private getDataUrl(parameter: IReviewPageParameter, roleType: number, planId: number, selectedSupervisorId: string): string {
    ...
    switch (roleType) {
      case RoleTypes.EMPLOYEE:
        return `${RATING_PAGE_DATA_URLs.data}/${planId}/${assetId}`;
      case RoleTypes.LINE_MANAGER:
        return `${RATING_PAGE_DATA_URLs.lineManagerData}/${employeeId}/${planId}/${assetId}`;
      case RoleTypes.EMPLOYEE_ACCEPT_REJECT:
        if (selectedSupervisorId === '0') {
          return `${RATING_PAGE_DATA_URLs.acceptRejectAllData}/${planId}/${assetId}`;
        } else {
          return `${RATING_PAGE_DATA_URLs.acceptRejectDataByLineManager}/${selectedSupervisorId}/${planId}/${assetId}`;
        }
      case RoleTypes.REVIEWER_ASSESSING:
        return `${RATING_PAGE_DATA_URLs.lineManagerData}/${employeeId}/${planId}/${assetId}`;
      case RoleTypes.REVIEWER_REVIEWING:
        if (selectedSupervisorId === '0') {
          return `${RATING_PAGE_DATA_URLs.reviewerAllData}/${employeeId}/${planId}/${assetId}`;
        } else {
          return `${RATING_PAGE_DATA_URLs.reviewerDataByLineManager}/${employeeId}/${planId}/${selectedSupervisorId}/${assetId}`;
        }
      case RoleTypes.MODERATION:
        if (selectedSupervisorId === '' || selectedSupervisorId === '0') {
          return `${RATING_PAGE_DATA_URLs.moderatorAllData}/${employeeId}/${planId}/${assetId}`;
        } else {
          return `${RATING_PAGE_DATA_URLs.moderatorDataByLineManager}/${employeeId}/${planId}/${selectedSupervisorId}/${assetId}`;
        }
      case RoleTypes.HR:
        if (selectedSupervisorId === '' || selectedSupervisorId === '0') {
          return `${RATING_PAGE_DATA_URLs.hrAllData}/${employeeId}/${planId}/${assetId}`;
        } else {
          return `${RATING_PAGE_DATA_URLs.hrDataByLineManager}/${employeeId}/${planId}/${selectedSupervisorId}/${assetId}`;
        }
    }
  }
  ...
  private getPageDataUrl(parameter: IReviewPageParameter, roleType: number, pageId: number): string {
    ...
    switch (roleType) {
      case RoleTypes.EMPLOYEE:
        return `${RATING_PAGE_DATA_URLs.pageData}/${pageId}`;
      case RoleTypes.LINE_MANAGER:
        return `${RATING_PAGE_DATA_URLs.lineManagerPageData}/${employeeId}/${pageId}`;
      case RoleTypes.EMPLOYEE_ACCEPT_REJECT:
        return `${RATING_PAGE_DATA_URLs.pageData}/${pageId}`;
      case RoleTypes.REVIEWER_ASSESSING:
        return `${RATING_PAGE_DATA_URLs.lineManagerTwoPageData}/${employeeId}/${pageId}`;
      case RoleTypes.REVIEWER_REVIEWING:
        return `${RATING_PAGE_DATA_URLs.reviewerPageData}/${employeeId}/${pageId}`;
      case RoleTypes.MODERATION:
        return `${RATING_PAGE_DATA_URLs.moderatorPageData}/${employeeId}/${pageId}`;
      case RoleTypes.HR:
        return `${RATING_PAGE_DATA_URLs.hrPageData}/${employeeId}/${pageId}`;
    }
  }
  ...
  private getSupervisorUrl(parameter: IReviewPageParameter, roleType: number, planId: number): string {
    ...
    switch (roleType) {
      ...
      case RoleTypes.LINE_MANAGER:
        return ``;
      case RoleTypes.EMPLOYEE_ACCEPT_REJECT:
        return `${RATING_PAGE_DATA_URLs.supervisors}/${planId}`;
      case RoleTypes.REVIEWER_ASSESSING:
        return ``;
      case RoleTypes.REVIEWER_REVIEWING:
        return `${RATING_PAGE_DATA_URLs.reviewerSupervisors}/${planId}/${employeeId}`;
      case RoleTypes.MODERATION:
        return `${RATING_PAGE_DATA_URLs.reviewerSupervisors}/${planId}/${employeeId}`;
      case RoleTypes.HR:
        return `${RATING_PAGE_DATA_URLs.reviewerSupervisors}/${planId}/${employeeId}`;
  ...
  private reOpenCompletedSectionService(pageId, employeeId, planId): string {
    return `${RATING_PAGE_DATA_URLs.reOpenCompletedSection}/${pageId}/${employeeId}/${planId}`;
  }
}
