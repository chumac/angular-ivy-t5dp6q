...
import { HRSummaryPageService } from './hr-summary-page.service';
...
@Component({
  selector: 'x365-fm-talent-hr-summary-page',
  templateUrl: './hr-summary-page.component.html',
  styleUrls: ['./hr-summary-page.component.scss']
})
export class HrSummaryPageComponent extends BaseFormComponent implements OnInit, OnDestroy, IReviewPageComponent {
  @Input() show: boolean;
  ...
  constructor(private router: Router, private utilService: UtilService, private store: Store<IAppState>, private reviewWorkflowProcessService: ReviewWorkflowProcessService, public fs: HRSummaryPageService, private dialog: MatDialog) {
    super();
  }
  ...
  completeReview() {
    ...
    if (this.parameter && this.parameter.reviewWorkflowProcess) {
      ...
      this.subscriptions['completeAppraisal'] =  this.reviewWorkflowProcessService.completeAppraisal(workflowProcessId, body).subscribe(data => {
        if (data.Success) {
          this.utilService.showToast('Complete Appraisal', `Appraisal was closed successfully.`, ToastTypes.SUCCESS);
          this.setReviewWorkflowData();
          this.formCompleteEmitter.emit();  // Notify host parent (appraisal form)

          this.router.navigate([`${STANDARD_ROUTES.hrQueue}`]);
        } else {
          this.utilService.showToast('Complete Appraisal: Error Occured', data.ErrorMessage, ToastTypes.ERROR);
        }
      }, (error) => {
        this.utilService.showToast('Complete Appraisal: Error Occured', `Something went wrong. Review not completed.`, ToastTypes.ERROR);
        this.completeProcessingStatus = false;
      }, () => {
         this.completeProcessingStatus = false;
      });
    } else {
      this.utilService.showToast('Complete Appraisal: Error Occured', `Something went wrong. Not enough data to proceed.`, ToastTypes.ERROR);
    }
  }
  ...
  getBody(): any {
    ...
    return {
      ...
      hr_comments: this.fs.comment.value
    }
  }
