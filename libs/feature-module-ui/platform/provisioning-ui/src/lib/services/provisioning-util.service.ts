...
import { formatDate, toastOptionsSuccess, toastOptionsInformation, toastOptionsWarning, toastOptionsError } from '@nutela/core-services';
import { IRolesTransform } from '../models/interfaces';

@Injectable({
  providedIn: 'root'
})
export class ProvisioningUtilService {
  ...
  transformToRolesDataList(
    list: any,
    rolenameProp?: string,
    sys_rolenameProp?: string
  ): IRolesTransform[] {
    let tempList: IRolesTransform[] = [];
    ...
    list.forEach(element => {
      ...
      if (rolenameProp) {
        role_name = this.fetchPropertyValue(element, rolenameProp);
      } else {
        role_name = element;
      }

      if (sys_rolenameProp) {
        sys_rolename = this.fetchPropertyValue(element, sys_rolenameProp);
      } else {
        sys_rolename = element;
      }

      let newObj = { role_name: role_name, sys_rolename: sys_rolename };

      tempList.push(newObj);
    });

    return <IRolesTransform[]>tempList;
  }
