...
import { HrReboardGuarantorsEditorService } from './hr-reboard-guarantors-editor.service';
...
import { isProcessingHrReboardGuarantor, ProcessingHrReboardGuarantor, SaveHrReboardGuarantor, NotProcessingHrReboardGuarantor, SaveUpdateHrReboardGuarantor } from '../../../../store/hr-reboard-data';


@Component({
  selector: 'x365-fm-workforce-hr-reboard-guarantors-editor',
  templateUrl: './hr-reboard-guarantors-editor.component.html',
  styleUrls: ['./hr-reboard-guarantors-editor.component.scss'],
  providers: [HrReboardGuarantorsEditorService],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class HrReboardGuarantorsEditorComponent  extends BaseFormComponent
  ...
  constructor(
    public fs: HrReboardGuarantorsEditorService,
  ...
  storeSelects() {
    this.isProcessing$ = this.store.pipe(select(isProcessingHrReboardGuarantor));
  }
  ...
  onSubmit() {
    ...
    if (this.fs.valid) {
      ...
      this.store.dispatch(new ProcessingHrReboardGuarantor());
      this.data ? this.store.dispatch(new SaveUpdateHrReboardGuarantor({ data: <IGuarantor>this.fs.value, recordId, employeeId: this.employeeId })) :
      this.store.dispatch(new SaveHrReboardGuarantor({ data: <IGuarantor>this.fs.value, employeeId: this.employeeId }));
    } else {
  ...
  onCancel() {
    this.store.dispatch(new NotProcessingHrReboardGuarantor());
    this.data = null;
    this.reset();
