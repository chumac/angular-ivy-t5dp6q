...
import { HrReboardContactEditorService } from './hr-reboard-contact-editor.service';
...
import { isHrReboardContactProcessing, ProcessingHrReboardContact, SaveHrReboardContact, NotProcessingHrReboardContact, SaveUpdateHrReboardContact } from '../../../../store/hr-reboard-data';

@Component({
  selector: 'x365-fm-workforce-hr-reboard-contact-editor',
  templateUrl: './hr-reboard-contact-editor.component.html',
  styleUrls: ['./hr-reboard-contact-editor.component.scss'],
  providers: [HrReboardContactEditorService],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class HrReboardContactEditorComponent extends BaseFormComponent
  ...
  constructor(
    public fs: HrReboardContactEditorService,
  ...
  storeSelects() {
    this.isProcessing$ = this.store.pipe(select(isHrReboardContactProcessing));
  ...
  setNOKCountryLists(data: IReboardContact) {
    let countryId = data.country_kin.nationality_id;
    let stateId = data.state_kin.state_id;
  ...
  onSubmit() {
    ...
    if (this.fs.valid) {
        this.store.dispatch(new ProcessingHrReboardContact());
        this.store.dispatch(new SaveUpdateHrReboardContact({ contactId: this.data.employeecontact_id, data: <IContact>this.fs.value, employeeId: this.employeeId }));
    } else {
  ...
  onCancel() {
    this.store.dispatch(new NotProcessingHrReboardContact());
    this.reset();
