...
import { ReboardRefereesEditorService } from './reboard-referees-editor.service';
import * as constants from '@nutela/shared/app-global';
import { ShowToast } from '@nutela/store/shared';
import { ISelectOption } from '@nutela/models/core-data';
import { FilePickerComponent, ImagePickerComponent, DialogService } from '@nutela/shared/ui';
import { isProcessingReboardReferee, ProcessingReboardReferee, SaveReboardReferee, NotProcessingReboardReferee, SaveUpdateReboardReferee } from '../../../../store/my-reboard-data';

@Component({
  selector: 'x365-fm-workforce-reboard-referees-editor',
  templateUrl: './reboard-referees-editor.component.html',
  styleUrls: ['./reboard-referees-editor.component.scss'],
  providers: [ReboardRefereesEditorService],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReboardRefereesEditorComponent extends BaseFormComponent
  ...
  constructor(
    public fs: ReboardRefereesEditorService,
  ...
  ngOnInit() {
    this.isProcessing$ = this.store.pipe(select(isProcessingReboardReferee));
  ...
  onSubmit() {
    ...
    if (this.fs.valid) {
      ...
      this.store.dispatch(new ProcessingReboardReferee());
      this.data ? this.store.dispatch(new SaveUpdateReboardReferee({ data: <IReferee>this.fs.value, recordId })) :
        this.store.dispatch(new SaveReboardReferee({ data: <IReferee>this.fs.value }));
    } else {
  ...
  onCancel() {
    this.store.dispatch(new NotProcessingReboardReferee());
    this.data = null;
    this.reset();
