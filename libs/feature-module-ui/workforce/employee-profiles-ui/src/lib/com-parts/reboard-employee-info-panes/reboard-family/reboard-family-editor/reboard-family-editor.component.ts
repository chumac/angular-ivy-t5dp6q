...
import { ReboardFamilyEditorService } from './reboard-family-editor.service';
...
import { isProcessingReboardFamily, ProcessingReboardFamily, SaveReboardFamily, NotProcessingReboardFamily, SaveUpdateReboardFamily } from '../../../../store/my-reboard-data';


@Component({
  selector: 'x365-fm-workforce-reboard-family-editor',
  templateUrl: './reboard-family-editor.component.html',
  styleUrls: ['./reboard-family-editor.component.scss'],
  providers: [ReboardFamilyEditorService],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReboardFamilyEditorComponent  extends BaseFormComponent
  ...
  constructor(
    public fs: ReboardFamilyEditorService,
  ...
  storeSelects() {
    this.isProcessing$ = this.store.pipe(select(isProcessingReboardFamily));
  ...
  onSubmit() {
    ...
    if (this.fs.valid) {
      ...
      this.store.dispatch(new ProcessingReboardFamily());
      this.data ? this.store.dispatch(new SaveUpdateReboardFamily({ data: this.fs.value, recordId })) :
      this.store.dispatch(new SaveReboardFamily({ data: this.fs.value }));
    } else {
  ...
  onCancel() {
    this.store.dispatch(new NotProcessingReboardFamily());
    this.data = null;
    this.reset();
