...
import { ReboardDependantsEditorService } from './reboard-dependants-editor.service';
...
import { isProcessingReboardDependant, ProcessingReboardDependant, SaveReboardDependant, NotProcessingReboardDependant, SaveUpdateReboardDependant } from '../../../../store/my-reboard-data';

@Component({
  selector: 'x365-fm-workforce-reboard-dependants-editor',
  templateUrl: './reboard-dependants-editor.component.html',
  styleUrls: ['./reboard-dependants-editor.component.scss'],
  providers: [ReboardDependantsEditorService],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReboardDependantsEditorComponent  extends BaseFormComponent
  ...
  constructor(
    public fs: ReboardDependantsEditorService,
  ...
  storeSelects() {
    this.isProcessing$ = this.store.pipe(select(isProcessingReboardDependant));
  ...
  onSubmit() {
    ...
    if (this.fs.valid) {
      ...
      this.store.dispatch(new ProcessingReboardDependant());
      this.data ? this.store.dispatch(new SaveUpdateReboardDependant({ data: <IDependant>this.fs.value, recordId })) :
        this.store.dispatch(new SaveReboardDependant({ data: <IDependant>this.fs.value }));
    } else {
  ...
  onCancel() {
    this.store.dispatch(new NotProcessingReboardDependant());
    this.data = null;
    this.reset();
