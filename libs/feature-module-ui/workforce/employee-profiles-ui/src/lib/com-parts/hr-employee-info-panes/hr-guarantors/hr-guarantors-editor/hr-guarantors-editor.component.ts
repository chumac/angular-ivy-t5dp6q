...
import { ProcessingGuarantor, SaveGuarantor, NotProcessingGuarantor, isProcessingGuarantor } from '../../../../store/employee-detailed-area';
...
@Component({
  selector: 'x365-fm-workforce-hr-guarantors-editor',
  templateUrl: './hr-guarantors-editor.component.html',
  styleUrls: ['./hr-guarantors-editor.component.scss'],
  providers: [GuarantorsEditorService],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class HrGuarantorsEditorComponent  extends BaseFormComponent
    ...
    if (this.fs.valid) {
      ...
      this.dialogService.confirmed().pipe(take(1)).subscribe(confirmed => {
        if (confirmed) {
          ...
          this.store.dispatch(new SaveGuarantor({ data: <IGuarantor>this.fs.value, recordId: recordId, editMode: this.inEditMode(), employeeId: this.employeeId }));
        }
      });
    } else {
