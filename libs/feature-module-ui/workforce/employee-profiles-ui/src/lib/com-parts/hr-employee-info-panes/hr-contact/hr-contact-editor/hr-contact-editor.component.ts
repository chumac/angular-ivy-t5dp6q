...
import { ImagePickerComponent, FilePickerComponent, IFileData, DialogService, DialogBoxCommandTypes } from '@nutela/shared/ui';
import { IEmployeesProfileState } from '../../../../store';
import { ProcessingContact, SaveContact, NotProcessingContact, isContactProcessing ,
         getResidentialStateList, getResidentialCityList, getPermanentStateList, getPermanentCityList,
         getNextOfKinStateList, getNextOfKinCityList, LoadResidentialStatesContact, LoadResidentialCitiesContact,
         LoadPermanentStatesContact, LoadPermanentCitiesContact, LoadNextOfKinStatesContact, LoadNextOfKinCitiesContact
       } from '../../../../store/employee-detailed-area';
import { map, take } from 'rxjs/operators';
import { getSelectOptionData, getCountries, allowPreferredEmailChange } from '@nutela/store/modules/foundation';


@Component({
  selector: 'x365-fm-workforce-hr-contact-editor',
  templateUrl: './hr-contact-editor.component.html',
  styleUrls: ['./hr-contact-editor.component.scss']
})
export class HrContactEditorComponent extends BaseFormComponent
  ...
  nationalityList: ISelectOption[]
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['data']) {
      // this.nationalityList = this.selectOptionData.Nationality;
  ...
  /// Subscriptions
  approvedDataSubscription: Subscription = null;
  /// Subscriptions
  ...
  onDuplicateAddressChecked(event, x) {
    this.fs.togglePermanentAddress(event.target.checked, this.data)
    this.setPermanentCountryListsByResidential(this.data);
  }


  getCountryData$(rowId: number): Observable<ISelectOption> {
    return this.selectOptionData$.pipe(
      map(d => d.Nationality.filter(v => v.value === rowId.toString())),
      map(e => e.shift()))
  }


  onFileSelected(data: IFileData) {
    if (data) {
      this.fs.patch({
        doc_extension: data.extension,
        doc_size: data.size,
        doc_mimetype: data.mimeType,
      });
    } else {
    ...
    if (this.fs.valid) {
      ...
      this.dialogService.confirmed().pipe(take(1)).subscribe(confirmed => {
        if (confirmed) {
          ...
          this.store.dispatch(new SaveContact({ employeeId: this.employeeId, employeeContactId: this.data.employeecontact_id, data: <IContact>this.fs.value }));
        }
      });
    } else {
  ...
  unsubscribe() {
    if (this.approvedDataSubscription) {
      this.approvedDataSubscription.unsubscribe();
    }
  }
