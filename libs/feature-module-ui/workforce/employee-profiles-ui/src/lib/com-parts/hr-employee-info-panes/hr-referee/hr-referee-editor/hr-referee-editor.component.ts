...
@Component({
  selector: 'x365-fm-workforce-hr-referee-editor',
  templateUrl: './hr-referee-editor.component.html',
  styleUrls: ['./hr-referee-editor.component.scss'],
  providers: [PersonalRefereesEditorService],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class HrRefereeEditorComponent extends BaseFormComponent
    ...
    if (this.fs.valid) {
      ...
      this.dialogService.confirmed().pipe(take(1)).subscribe(confirmed => {
        if (confirmed) {
          ...
          this.store.dispatch(new SaveReferee({ data: <IReferee>this.fs.value, recordId: recordId, editMode: this.inEditMode(), employeeId: this.employeeId }));
        }
      });
    } else {
