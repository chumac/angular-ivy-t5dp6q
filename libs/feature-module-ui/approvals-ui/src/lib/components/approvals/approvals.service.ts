...
import { OBJECTIVE_APPROVAL_URLs } from '@nutela/feature-module-ui/talent/performance-ui';
...
export class ApprovalsService {
  ...
  setToolTip(fieldLabel: string) {
    this.toolTips = fieldLabel;
  }

  setupToolTips(res) {
    let toShow = '';
    res.forEach(element => {
      toShow = toShow.concat(element.label, '\n')
    })
    return toShow;
  }

  loadExplicitlyWorkflowMetadata(msgId: number): Observable<IApiResult>{
    return this.apiService.read(`${OBJECTIVE_APPROVAL_URLs.getworkflowdata}/${msgId}`);
  }

  loadResignationHRFinalizeMetadata(msgId: number): Observable<IApiResult> {
    return this.apiService.read(`${APPROVE_DATA_URLs.hrFinalizeWorkflowUrl}/${msgId}`);
  };

  loadReboardWorkflowMetadata(msgId: number): Observable<IApiResult>{
    return this.apiService.read(`${APPROVE_DATA_URLs.getReboardWorkflow}/${msgId}`);
  }
}
