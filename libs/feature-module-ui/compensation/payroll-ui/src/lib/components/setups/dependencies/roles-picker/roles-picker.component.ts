...
import {
  IRoles,
  IRolesTransform
} from 'libs/feature-module-ui/platform/provisioning-ui/src/lib/models/interfaces';

@Component({
  selector: 'x365-fm-payrl-pay-group-roles-picker',
    ...
    if (this.rolesData) {
      ...
      for (var i = this.rolesData.length - 1; i >= 0; i--) {
        for (var j = 0; j < this.rolesList.selectedItems.length; j++) {
          if (
            this.rolesData[i] &&
            this.rolesData[i].role_name ===
              this.rolesList.selectedItems[j].role_name
          ) {
  ...
  removeSelectedRows() {
    ...
    for (var i = this.selectedRolesList.length - 1; i >= 0; i--) {
      for (var j = 0; j < this.addedRolesList.selectedItems.length; j++) {
        if (
          this.selectedRolesList[i] &&
          this.selectedRolesList[i].role_name ===
            this.addedRolesList.selectedItems[j].role_name
        ) {
